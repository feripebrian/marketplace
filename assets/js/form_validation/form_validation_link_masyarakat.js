$(document).ready(function(){
	...
	$('#form-link-masyarakat').find('.pesan .pesan-text').hide();

	// VALIDASI select, input, textarea
	$('#form-link-masyarakat').find('select, input, textarea').each(function(){
		$(this).blur(function(){
			if( !$(this).val() ) {
	...
	// VALIDASI NAMA LINK
	$('#form-link-masyarakat').find('#nama_link').blur(function(){
	...
	// VALIDASI LINK
	$('#form-link-masyarakat').find('#link').blur(function(){
		let link = $(this).val();
		let len   = link.length;
	...
	$('#form-link-masyarakat').find('button#simpan-link-masyarakat').click(function(){
		aksi = "simpan";
	});
	$('#form-link-masyarakat').find('button#ubah-link-masyarakat').click(function(){
		aksi = "ubah";
	});

	$('#form-link-masyarakat').submit(function(evt){
		evt.preventDefault();
		let valid = true; 
		var form = $('form')[0];
		...
		if( valid === true ) {
			if( aksi == "simpan" ) {
				...
				$.ajax({
					type: "POST",
					url: "link_masyarakat/aksi_link_masyarakat.php?linkmasyarakat=tambah",
					data: formData,
					contentType: false,
					processData: false,
				...
				$.ajax({
					type: "POST",
					url: "link_masyarakat/aksi_link_masyarakat.php?linkmasyarakat=ubah",
					data: formData,
					contentType: false,
					processData: false,
					success: function(result){
