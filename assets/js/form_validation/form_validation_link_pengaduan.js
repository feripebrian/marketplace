$(document).ready(function(){
	...
	$('#form-link-pengaduan').find('.pesan .pesan-text').hide();

	// VALIDASI select, input, textarea
	$('#form-link-pengaduan').find('select, input, textarea').each(function(){
		$(this).blur(function(){
			if( !$(this).val() ) {
	...
	// VALIDASI NAMA LINK
	$('#form-link-pengaduan').find('#nama_link').blur(function(){
	...
	// VALIDASI LINK
	$('#form-link-pengaduan').find('#link').blur(function(){
		let link = $(this).val();
		let len   = link.length;
	...
	$('#form-link-pengaduan').find('button#simpan-link-pengaduan').click(function(){
		aksi = "simpan";
	});
	$('#form-link-pengaduan').find('button#ubah-link-pengaduan').click(function(){
		aksi = "ubah";
	});

	$('#form-link-pengaduan').submit(function(evt){
		evt.preventDefault();
		let valid = true;
		...
		if( valid === true ) {
			if( aksi == "simpan" ) {
				...
				$.ajax({
					type: "POST",
					url: "link_pengaduan/aksi_link_pengaduan.php?linkpengaduan=tambah",
					data: dataForm,
				...
				$.ajax({
					type: "POST",
					url: "link_pengaduan/aksi_link_pengaduan.php?linkpengaduan=ubah",
					data: dataForm,
					success: function(result){
