$(document).ready(function(){
	...
	$('#form-link-surat').find('.pesan .pesan-text').hide();

	// VALIDASI select, input, textarea
	$('#form-link-surat').find('select, input, textarea').each(function(){
		$(this).blur(function(){
			if( !$(this).val() ) {
	...
	// VALIDASI NAMA LINK
	$('#form-link-surat').find('#nama_link').blur(function(){
	...
	// VALIDASI LINK
	$('#form-link-surat').find('#link').blur(function(){
		let link = $(this).val();
		let len   = link.length;
	...
	$('#form-link-surat').find('button#simpan-link-surat').click(function(){
		aksi = "simpan";
	});
	$('#form-link-surat').find('button#ubah-link-surat').click(function(){
		aksi = "ubah";
	});

	$('#form-link-surat').submit(function(evt){
		evt.preventDefault();
		let valid = true;
		...
		if( valid === true ) {
			if( aksi == "simpan" ) {
				...
				$.ajax({
					type: "POST",
					url: "link_surat/aksi_link_surat.php?linksurat=tambah",
					data: dataForm,
				...
				$.ajax({
					type: "POST",
					url: "link_surat/aksi_link_surat.php?linksurat=ubah",
					data: dataForm,
					success: function(result){
