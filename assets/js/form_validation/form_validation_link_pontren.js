$(document).ready(function(){
	...
	$('#form-link-pontren').find('.pesan .pesan-text').hide();

	// VALIDASI select, input, textarea
	$('#form-link-pontren').find('select, input, textarea').each(function(){
		$(this).blur(function(){
			if( !$(this).val() ) {
	...
	// VALIDASI NAMA LINK
	$('#form-link-pontren').find('#nama_link').blur(function(){
	...
	// VALIDASI LINK
	$('#form-link-pontren').find('#link').blur(function(){
		let link = $(this).val();
		let len   = link.length;
	...
	$('#form-link-pontren').find('button#simpan-link-pontren').click(function(){
		aksi = "simpan";
	});
	$('#form-link-pontren').find('button#ubah-link-pontren').click(function(){
		aksi = "ubah";
	});

	$('#form-link-pontren').submit(function(evt){
		evt.preventDefault();
		let valid = true;
		...
		if( valid === true ) {
			if( aksi == "simpan" ) {
				...
				$.ajax({
					type: "POST",
					url: "link_pontren/aksi_link_pontren.php?linkpontren=tambah",
					data: dataForm,
				...
				$.ajax({
					type: "POST",
					url: "link_pontren/aksi_link_pontren.php?linkpontren=ubah",
					data: dataForm,
					success: function(result){
