$(document).ready(function(){
	...
	$('#form-link-penmad').find('.pesan .pesan-text').hide();

	// VALIDASI select, input, textarea
	$('#form-link-penmad').find('select, input, textarea').each(function(){
		$(this).blur(function(){
			if( !$(this).val() ) {
	...
	// VALIDASI NAMA LINK
	$('#form-link-penmad').find('#nama_link').blur(function(){
	...
	// VALIDASI LINK
	$('#form-link-penmad').find('#link').blur(function(){
		let link = $(this).val();
		let len   = link.length;
	...
	$('#form-link-penmad').find('button#simpan-link-penmad').click(function(){
		aksi = "simpan";
	});
	$('#form-link-penmad').find('button#ubah-link-penmad').click(function(){
		aksi = "ubah";
	});

	$('#form-link-penmad').submit(function(evt){
		evt.preventDefault();
		let valid = true;
		...
		if( valid === true ) {
			if( aksi == "simpan" ) {
				...
				$.ajax({
					type: "POST",
					url: "link_penmad/aksi_link_penmad.php?linkpenmad=tambah",
					data: dataForm,
				...
				$.ajax({
					type: "POST",
					url: "link_penmad/aksi_link_penmad.php?linkpenmad=ubah",
					data: dataForm,
					success: function(result){
