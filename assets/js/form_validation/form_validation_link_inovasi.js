$(document).ready(function(){
	...
	$('#form-link-inovasi').find('.pesan .pesan-text').hide();

	// VALIDASI select, input, textarea
	$('#form-link-inovasi').find('select, input, textarea').each(function(){
		$(this).blur(function(){
			if( !$(this).val() ) {
	...
	// VALIDASI NAMA LINK
	$('#form-link-inovasi').find('#nama_link').blur(function(){
	...
	// VALIDASI LINK
	$('#form-link-inovasi').find('#link').blur(function(){
		let link = $(this).val();
		let len   = link.length;
	...
	$('#form-link-inovasi').find('button#simpan-link-inovasi').click(function(){
		aksi = "simpan";
	});
	$('#form-link-inovasi').find('button#ubah-link-inovasi').click(function(){
		aksi = "ubah";
	});

	$('#form-link-inovasi').submit(function(evt){
		evt.preventDefault();
		let valid = true; 
		var form = $('form')[0];
		...
		if( valid === true ) {
			if( aksi == "simpan" ) {
				...
				$.ajax({
					type: "POST",
					url: "link_inovasi/aksi_link_inovasi.php?linkinovasi=tambah",
					data: formData,
					contentType: false,
					processData: false,
				...
				$.ajax({
					type: "POST",
					url: "link_inovasi/aksi_link_inovasi.php?linkinovasi=ubah",
					data: formData,
					contentType: false,
					processData: false,
					success: function(result){
