$(document).ready(function(){
	...
	$('#form-link-survey').find('.pesan .pesan-text').hide();

	// VALIDASI select, input, textarea
	$('#form-link-survey').find('select, input, textarea').each(function(){
		$(this).blur(function(){
			if( !$(this).val() ) {
	...
	// VALIDASI NAMA LINK
	$('#form-link-survey').find('#nama_link').blur(function(){
	...
	// VALIDASI LINK
	$('#form-link-survey').find('#link').blur(function(){
		let link = $(this).val();
		let len   = link.length;
	...
	$('#form-link-survey').find('button#simpan-link-survey').click(function(){
		aksi = "simpan";
	});
	$('#form-link-survey').find('button#ubah-link-survey').click(function(){
		aksi = "ubah";
	});

	$('#form-link-survey').submit(function(evt){
		evt.preventDefault();
		let valid = true;
		...
		if( valid === true ) {
			if( aksi == "simpan" ) {
				...
				$.ajax({
					type: "POST",
					url: "link_survey/aksi_link_survey.php?linksurvey=tambah",
					data: dataForm,
				...
				$.ajax({
					type: "POST",
					url: "link_survey/aksi_link_survey.php?linksurvey=ubah",
					data: dataForm,
					success: function(result){
